...
namespace Dissonance.Editor.Windows
{
    ...
    internal class EditorMetadata
    {
        ...
        [NotNull] internal static string GetQueryStringWithIntegrations([NotNull] string utmMedium)
        {
            return GetQueryString(
                utmMedium,
                IntegrationMetadata.FindIntegrations().Select(a => new KeyValuePair<string, string>(Uri.EscapeUriString(a.Id), Uri.EscapeUriString(a.ReleasedWithDissonanceVersion.ToString())))
            );
        }
